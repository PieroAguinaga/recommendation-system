...
with torch.no_grad():
    all_video_emb = model.get_all_video_embeddings()
...
for start in range(0, N_USERS, BATCH_USERS):
    ...
    with torch.no_grad():
        u_emb = model.user_emb(batch_user_idx)
        u_bias = model.user_bias(batch_user_idx).squeeze()

        scores = torch.sigmoid(
            u_emb @ all_video_emb.T
            + u_bias.unsqueeze(1)
...
# ⚠️ Ajusta este path si tienes otro archivo de test
TEST_PATH = OUT_DIR / "test.csv"
test_df = pd.read_csv(TEST_PATH)

test_data = test_df[['user_id', 'video_id', 'label']].copy()

# Filtrar IDs conocidos
test_data = test_data[
    test_data['user_id'].isin(user2idx.keys()) &
    test_data['video_id'].isin(video2idx.keys())
].copy()
...
for K in [10, 50, 100]:
    ...
    print(
        f"Precision@{K}: {results_k['precision'].mean():.4f} | "
